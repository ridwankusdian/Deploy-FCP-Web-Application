...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	...
	kategoriID,
		salah := strconv.Atoi(c.Param("id"))
	if salah != nil {
		c.JSON(http.StatusBadRequest,
			model.ErrorResponse{Error: "invalid Category ID"})
		return
	}
	// Buat Variabel baru
	var kategoriBaru model.Category
	if salah := c.ShouldBindJSON(&kategoriBaru); salah != nil {
		c.JSON(http.StatusBadRequest,
			model.ErrorResponse{Error: salah.Error()})
		return
	}
	// Set Category ID
	kategoriBaru.ID = kategoriID
	// func Update dari Service
	salah = ct.categoryService.Update(kategoriID, kategoriBaru)
	if salah != nil {
	...
	c.JSON(http.StatusOK,
		model.SuccessResponse{Message: "category update success"})
...
func (ct *categoryAPI) DeleteCategory(c *gin.Context) {
	...
	if salah != nil {
		c.JSON(http.StatusBadRequest,
			model.ErrorResponse{Error: "Invalid task ID"})
		return
	}
	// func Delete dari Service
	salah = ct.categoryService.Delete(kategoriID)
	if salah != nil {
		c.JSON(http.StatusInternalServerError,
			model.ErrorResponse{Error: salah.Error()})
		return
	...
	// Respon Pesan
	c.JSON(http.StatusOK,
		model.SuccessResponse{Message: "category delete success"})
...
func (ct *categoryAPI) GetCategoryList(c *gin.Context) {
	// func GetList dari Service
	kategori,
		salah := ct.categoryService.GetList()
	if salah != nil {
		c.JSON(http.StatusInternalServerError,
			model.ErrorResponse{Error: salah.Error()})
		return
